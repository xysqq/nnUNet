...
if __name__ == '__main__':
    ...
    out_dir = "/home/xys/nnUNet/nnUNet_raw/Dataset003_organ_at_risk_double_CT"
    ...
    images_contrast_path_list = glob(f"{images_dir}/*/image_contrast.nii.gz")
    labels_path_list = glob(f"{labels_dir}/Task001/*")
    for image_path, image_contrast_path, label_path in tqdm(
            zip(images_path_list, images_contrast_path_list, labels_path_list)):
        image_id = f"{Path(image_path).parent.name}_0000.nii.gz"
        image_contrast_id = f"{Path(image_contrast_path).parent.name}_0001.nii.gz"
        label_id = f"{Path(label_path).name.split('.')[0]}.nii.gz"
        copy_file(image_path, f"{out_dir}/imagesTr/{image_id}")
        copy_file(image_contrast_path, f"{out_dir}/imagesTr/{image_contrast_id}")
        copy_file(label_path, f"{out_dir}/labelsTr/{label_id}")

    dataset_json = {"channel_names": {
        "0": "CT", "1": "contrast-enhanced CT"
