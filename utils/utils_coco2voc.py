...
def annsToSeg(anns, coco_instance):
    ...
    for i, mask in enumerate(masks):
        class_seg = np.where(
            class_seg > 0, class_seg, mask * (anns[i]['category_id'] + 1))
...
def annsToMask(anns, h, w):
    ...
    masks = []
    anns = sorted(
        anns, key=lambda x: x['area']
    )  # Smaller items first, so they are not covered by overlapping segs
    for ann in anns:
...
def coco2voc(anns_file, target_folder, n=None, compress=True):
    ...
    for i in coco_imgs:
        img_filename = Path(coco_imgs[i]['file_name']).stem
        anns_ids = coco_instance.getAnnIds(i)
        ...
        Image.fromarray(class_seg).convert("L").save(
            class_target_path + '/' + str(img_filename) + '.png')
        Image.fromarray(instance_seg).convert("L").save(
            instance_target_path + '/' + str(img_filename) + '.png')

        if compress:
            np.savez_compressed(
                os.path.join(id_target_path, str(img_filename)), id_seg)
        else:
            np.save(
                os.path.join(id_target_path,
                             str(img_filename) + '.npy'), id_seg)

        image_id_list.write(str(img_filename) + '\n')

        if i % 100 == 0 and i > 0:
            print(
                str(i) + " annotations processed" + " in " +
