...
def ct_dicom2img(origin, body_mask, astype='float32', center=0, width=512):
    ct_image = dicom2img(origin, center=center, width=width).astype(astype)
    ct_image[~body_mask] = 0
    return ct_image


def load_rtstruct_file(dicom_series_path, rt_struct_path):
    # 创建RTStructBuilder对象，用于解析RTSTRUCT文件
    rtstruct = RTStructBuilder.create_from(
        dicom_series_path=str(dicom_series_path), rt_struct_path=str(rt_struct_path))
    roi_names = rtstruct.get_roi_names()

    # 只保留Body区域，去除检查台阴影
    if 'BODY' in roi_names:
        body_masks = rtstruct.get_roi_mask_by_name('BODY')
    elif 'Body' in roi_names:
        body_masks = rtstruct.get_roi_mask_by_name('Body')
    else:
        body_masks = np.zeros((512, 512, len(rtstruct.series_data)),  dtype=bool)
        print(f"{dicom_series_path}不含body")
    return rtstruct, roi_names, body_masks


def get_patient_id_from_dicom(ct_dicom_dir):
    if platform.system() == 'Windows':
        patient_id = str(Path(ct_dicom_dir)).split('\\')[-5]
    else:
        patient_id = str(Path(ct_dicom_dir)).split('/')[-5]
    return patient_id
...
def image_normalize(image):
    # 将ndarray对象的最小值和最大值缩放到0到255之间
    image_scaled = (image - np.min(image)) / (np.max(image) - np.min(image)) * 255

    # 将值限制在0到255之间，确保其范围在0到255之间
...
def image_inverse_normalize(image, min_value, max_value):
    # 将图像的数据类型转换为float32，以避免溢出
    image = image.astype(np.uint32)

    # 将图像的值缩放到归一化前的范围
    image_inverse = (image / 255) * (max_value - min_value) + min_value

    return image_inverse


def load_mri_dicom_volumes(mri_dicom_dirs):
    if isinstance(mri_dicom_dirs, list):
        total_mri_volumes = []
        for mri_dicom_dir in mri_dicom_dirs:
            mri_volumes = _load_mri_dicom_volumes(mri_dicom_dir)
            total_mri_volumes.append(mri_volumes)
        return total_mri_volumes
    else:
        mri_volumes = _load_mri_dicom_volumes(mri_dicom_dirs)
        return mri_volumes


def _load_mri_dicom_volumes(mri_dicom_dir):
    ...
    sorted_mr_slice_location = sorted(mri_slice_location.items(), key=lambda x: x[1])

    mri_volumes = [pydicom.dcmread(mri_dicom_path).pixel_array for mri_dicom_path, slice_location in
                   sorted_mr_slice_location]
...
def _pixel_process(ds, pixel_array, center=None, width=None):
    """
    Process the images
    ...
    if center is not None and width is not None:
        pixel_array = _get_lut_value_linear_exact_normalized(
            pixel_array, width, center)
        return pixel_array
    # elif 'VOILUTFunction' in ds and ds.VOILUTFunction == 'SIGMOID':
    ...
    #     pixel_array = _get_lut_value_linear_exact(
    #         pixel_array, window_width, window_center)
    # else:
...
def _get_lut_value_linear_exact_normalized(data, window, level):
    ...
    """
    data = np.piecewise(
        data, [data <= (level - window / 2), data > (level + window / 2)],
...
def _ds_to_file(
        file_path, target_root, filetype, anonymous=None, patient_dict=None):
    """
    ...
    ########################
    # Color data already be converted by RGB implicitly by utils
    ...
    full_export_fp_fn = _get_export_file_path(
        ds, file_path, target_root, filetype, anonymous, patient_dict)
    # make dir
...
def _dicom_convertor(
    ...
    target_root, dicom_file_list = _get_root_get_dicom_file_list(
        origin, target_root)
    ...
    if anonymous == True:
        full_path_dict = _get_anonymous_full_path_dict(
            dicom_file_list, target_root, filetype)
    else:
        full_path_dict = None
    ...
    if multiprocessing == True:
        with concurrent.futures.ProcessPoolExecutor() as executor:
            return_future = [
                executor.submit(
                    _ds_to_file, file_path, target_root, filetype, anonymous,
                    full_path_dict) for file_path in dicom_file_list
            ]
            return_message = [future.result() for future in return_future]
    else:
        return_message = [
            _ds_to_file(
                file_path, target_root, filetype, anonymous, full_path_dict)
...
def _get_root_get_dicom_file_list(origin_input, target_root):
    ...
    if isinstance(origin_input, list) or isinstance(origin_input, tuple):
        origin_list = [
            Path(ori) for ori in origin_input
        ]  # make all items as path
    else:
...
def _get_export_file_path(
    ...
    # if no anonymous
        ...
        full_export_fp_fn = target_root / Path(today_str) / Path(
            f"{PatientID}_{filetype}") / Path(
            f"{StudyDate}_{StudyTime}_{Modality}_{AccessionNumber}"
        ) / Path(f"{SeriesNumber}_{InstanceNumber}.{filetype}")
...
def _get_anonymous_full_path_dict(dicom_file_list, target_root, file_type):
    ...
    for file_path in dicom_file_list:
        ...
        if PatientID not in patient_dict:
            ...
            patient_dict[PatientID]['patient_num'] = patient_dict[
                'last_pt_num']
        ...
        if AccessionNumber not in patient_dict[PatientID]:
            ...
            patient_dict[PatientID][AccessionNumber] = patient_dict[PatientID][
                'last_study_num']
        ...
        full_file_path = target_root / Path(today_str) / Path(
            patient_folder_name) / Path(study_folder_name) / Path(file_name)
